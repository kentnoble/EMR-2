...
app.controller('adultmedicineController',['$scope', '$location', '$window', '$filter','$routeParams','toastr', '$confirm','rfc4122','filterFilter','$modal','$log',
    'adultmedDataFactory', 'encounterDataFactory', 'transactionDataFactory', 'inventoryDataFactory', 'prescriptionDataFactory',
    ...
    function($scope, $location, $window, $filter, $routeParams, toastr, $confirm, rfc4122,filterFilter, $modal, $log,
        ...
        $scope.adultmedicine.Symptoms_AdultMed = '';
        $scope.adultmedicine.PainLocation_AdultMed = '';
        ...
        //load transactions Adult Medicines
        if($routeParams.id){
            $scope.id = $routeParams.id;
            adultmedDataFactory.getAdultMed($routeParams.id).then(function(data){
                $scope.transaction = data.data[0];
                if($scope.transaction !== undefined) {
                    $scope.adultmedicine = $scope.transaction.adultmedicine;
                    for(var i = 0; $scope.adultmedicine.Symptoms_AdultMed.split('|').length - 1 > i; i++){
                        $scope.selectedSymptoms.push($scope.adultmedicine.Symptoms_AdultMed.split('|')[i]);
                    }
                    for(var i = 0; $scope.adultmedicine.PainLocation_AdultMed.split('|').length - 1 > i; i++){
                        $scope.selectedPainLocation.push($scope.adultmedicine.PainLocation_AdultMed.split('|')[i]);
                    }
        ...
        $scope.addTransaction = function(){
            if ($scope.patientToServe) {
                ...
                transaction.Module_TransH = 'adultmedicine';
                transaction.SysFK_Patient_Trans = $scope.patient.SysPK_Patient;
                transaction.SysFK_Doctor_Trans = $scope.doctor.SysPK_Doctor;

                var adultmedicine = $scope.adultmedicine;
                adultmedicine.SysPK_AdultMed = rfc4122.v4();
                adultmedicine.SysFK_Trans_AdultMed = transaction.SysPK_Trans;
                ...
                department.SysFK_Dept_Department = adultmedicine.SysPK_AdultMed;
                department.Module_Department = 'adultmedicine';
                department.Particulars_Department = adultmedicine.ReasonForEncounter_AdultMed;

                for(var i = 0; $scope.selectedSymptoms.length > i; i++){
                    adultmedicine.Symptoms_AdultMed += $scope.selectedSymptoms[i] + "|";
                }
                for(var i = 0; $scope.selectedPainLocation.length > i; i++){
                    adultmedicine.PainLocation_AdultMed += $scope.selectedPainLocation[i] + "|";
                }

                transactionDataFactory.addTransaction(transaction).then(function () {});
                adultmedDataFactory.addAdultMed(adultmedicine).then(function () {});
