...
(function(window, angular, undefined) {
  ...
  angular.module('satellizer', [])
    .constant('SatellizerConfig', {
      ...
      tokenName: 'token',
      tokenPrefix: 'accesssoft', //satellizer
    ...
    .provider('$auth', ['SatellizerConfig', function(config) {
      ...
      this.$get = [
        ...
        function($q, shared, local, oauth) {
          ...
          $auth.setCookie = function(user) {
            return shared.setCookie(user)
          };

          $auth.removeCookie = function() {
            return shared.removeCookie();
          };
    ...
    .factory('SatellizerShared', [
      ...
      function($q, $window, $log, utils, config, storage, $cookieStore, $rootScope, Base64) {
        ...
        Shared.setCookie  = function (user) {
          console.log(user);
          $rootScope.globals = {
            currentUser: {
              n: Base64.encode(user.Name_User),
              sys: Base64.encode(user.SysPK_User),
              fk: Base64.encode(user.SysFK_User_User),
              rol: Base64.encode(user.AccessRights_User)
            }
          };
